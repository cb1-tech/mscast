"""MSCAST POC - 58: BATCH C4 fix - project status print (dict aggregate syntax), BRM certified sample."""
import frappe

log = lambda m: print("[C4c] " + m, flush=True)
...
{% set sis = frappe.get_all("Sales Invoice", filters={"project": doc.name, "docstatus": 1, "is_return": 0}, fields=["grand_total","outstanding_amount"]) %}
{% set billed = sis | sum(attribute="grand_total") %}
{% set outst = sis | sum(attribute="outstanding_amount") %}
<table class="ms">
  ...
  <tbody>
    ...
    <tr><td>Bought-out billed by suppliers</td>
      <td class="num">{{ frappe.utils.fmt_money(frappe.get_all("Purchase Invoice Item", filters={"project": doc.name, "docstatus": 1}, fields=["amount"]) | sum(attribute="amount")) }}</td>
      <td>Dispatch memos raised</td>
      <td>{{ frappe.get_all("MSCAST MDM", filters={"project": doc.name}) | length }}</td></tr>
    <tr><td>Open client claims</td>
      <td class="num">{{ frappe.utils.fmt_money(frappe.get_all("MSCAST Client Claim", filters={"project": doc.name, "status": ["not in", ["Settled","Rejected"]]}, fields=["claim_amount"]) | sum(attribute="claim_amount")) }}</td>
      <td>Commissioning</td>
